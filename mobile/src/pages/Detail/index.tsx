...
const Detail = () => {
  ...
  function handleComposeEmail(){
    MailComposer.composeAsync({
      subject: 'Interesse no descartar de resíduos',
      recipients: [point.email]
    });
  }

  function handleWhatsapp(){
    Linking.openURL(`whatsapp://send?phone=${point.whatsapp}&text=Tenho interesse em descartar residuos. Como devo proceder?`)
  }
    ...
    <SafeAreaView style={{flex:1, paddingTop:30}}>
      <View style={styles.container}>
        ...
        {point.items.map(item =>(
          <Text key={item.id} style={styles.pointItems}>{item.title}</Text>
        ))}
